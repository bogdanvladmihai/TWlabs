...
function addOnClockEvents() {
  ...
  for (let i = 0; i < N; i++) {
    for (let j = 0; j < M; j++) {
      ...
      target = getNthChild(getNthChild(table, i), j);
...
window.onload = function(){
  ...
  let raimbow = document.getElementById("curcubeu");
  raimbow.addEventListener("click", function() {
    rainbow(Math.floor(Math.random() * 2) == 1 ? true : false);
  });
  ...
  clear.addEventListener("click", function() {
    table = document.getElementById("table");
    let N = table.childElementCount, M = table.firstChild.childElementCount;
    drawRect(0, 0, N - 1, M - 1, "rgb(255,255,255)");
  });
}